...
export async function initCommand() {
  ...
  console.log(chalk.hex('#10B981')('└─ ') + chalk.hex('#EC4899')('✨ Ready to add components with ') + chalk.yellow('ngsui add [component]') + chalk.hex('#EC4899')(' and blocks with ') + chalk.yellow('ngsui add block [block]') + chalk.hex('#10B981')(' ─┘'));
  console.log('');
  ...
  try {
    ...
    if (shouldInstallPackage) {
      try {
        ...
        execSync('npm install class-variance-authority clsx tailwind-merge @angular/cdk @angular/animations --legacy-peer-deps', { stdio: 'inherit' });
      } catch (error) {
...
export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
    ...
    try {
      ...
      const appConfigPath = appConfigPaths.find(path => fs.pathExistsSync(path));

      if (appConfigPath) {
        spinner.text = 'Updating app.config.ts with animations provider...';

        let appConfigContent = await fs.readFile(appConfigPath, 'utf8');
            ...
            if (existingAnimationsImport) {
              ...
              if (!imports.includes('provideAnimationsAsync')) {
                ...
                appConfigContent = appConfigContent.replace(
                  importRegex,
                  `import { ${imports.join(', ')} } from '@angular/platform-browser/animations/async';`
                );
              }
            } else {
              // Add new import
      ...
      console.log('');
      console.log(chalk.bgYellow.black(' WARNING ') + ' ' + chalk.yellow(`Could not update app.config.ts: ${error instanceof Error ? error.message : String(error)}`));
      console.log(chalk.gray('You may need to manually add provideAnimationsAsync to your app.config.ts:'));
      console.log(chalk.gray('  import { provideAnimationsAsync } from \'@angular/platform-browser/animations/async\';'));
      console.log(chalk.gray('  providers: [..., provideAnimationsAsync()]'));
      console.log('');
    }
...
`;
    ...
    const stylesPath = await fs.pathExists(stylesScssPath) ? stylesScssPath : stylesCssPath;
    ...
    if (existingStyles) {
      ...
      if (!isAngularSuperUIStyles && existingContent.trim().length > 0) {
        ...
        console.log('');
        console.log(chalk.hex('#FF6B6B')('╔═══════════════════════════════════════════════════════════════════╗'));
        console.log(chalk.hex('#FF6B6B')('║') + chalk.hex('#FFE66D')(padding('⚠️  STYLES.CSS REPLACEMENT WARNING ⚠️', 67)) + chalk.hex('#FF6B6B')('║'));
        console.log(chalk.hex('#FF6B6B')('║') + chalk.hex('#A8E6CF')(padding('', 67)) + chalk.hex('#FF6B6B')('║'));
        console.log(chalk.hex('#FF6B6B')('║') + chalk.hex('#C7CEEA')(padding(`Found existing styles in: ${stylesPath}`, 67)) + chalk.hex('#FF6B6B')('║'));
        console.log(chalk.hex('#FF6B6B')('║') + chalk.hex('#FF8B94')(padding('Angular SuperUI will REPLACE your current styles', 67)) + chalk.hex('#FF6B6B')('║'));
        console.log(chalk.hex('#FF6B6B')('║') + chalk.hex('#FF8B94')(padding('with TailwindCSS v4 and theme variables', 67)) + chalk.hex('#FF6B6B')('║'));
        console.log(chalk.hex('#FF6B6B')('║') + chalk.hex('#A8E6CF')(padding('', 67)) + chalk.hex('#FF6B6B')('║'));
        console.log(chalk.hex('#FF6B6B')('║') + chalk.bold.hex('#FFE66D')(padding('⚡ BACKUP YOUR STYLES FIRST! ⚡', 67)) + chalk.hex('#FF6B6B')('║'));
        console.log(chalk.hex('#FF6B6B')('╚═══════════════════════════════════════════════════════════════════╝'));
        console.log('');

        const { replaceStyles } = await inquirer.prompt([
          {
            type: 'confirm',
            name: 'replaceStyles',
        ...
        if (!replaceStyles) {
          console.log(chalk.yellow('⚠️  Initialization cancelled. Your styles.css was not modified.'));
          console.log(chalk.gray('💡 To complete setup later, backup your styles and run: ngsui init'));
          return;
        }
    ...
    console.log(chalk.hex('#FF6B6B')('│                                                                    │'));
    console.log(chalk.hex('#FF6B6B')('│  ') + chalk.hex('#FFE66D')('📦 Components:') + chalk.hex('#FF6B6B')('                                               │'));
    console.log(chalk.hex('#FF6B6B')('│  ') + chalk.hex('#FFE66D')('1. Add your first component: ') + chalk.bold.hex('#A8E6CF')('ngsui add badge') + chalk.hex('#FF6B6B')('        │'));
    console.log(chalk.hex('#FF6B6B')('│  ') + chalk.hex('#FFE66D')('2. Add multiple components: ') + chalk.bold.hex('#A8E6CF')('ngsui add accordion alert') + chalk.hex('#FF6B6B')('  │'));
    console.log(chalk.hex('#FF6B6B')('│  ') + chalk.hex('#FFE66D')('3. Browse all components:  ') + chalk.bold.hex('#A8E6CF')('ngsui list') + chalk.hex('#FF6B6B')('                  │'));
    console.log(chalk.hex('#FF6B6B')('│                                                                    │'));
    console.log(chalk.hex('#FF6B6B')('│  ') + chalk.hex('#FFE66D')('🎨 UI Blocks:') + chalk.hex('#FF6B6B')('                                                  │'));
    console.log(chalk.hex('#FF6B6B')('│  ') + chalk.hex('#FFE66D')('1. Add hero section:        ') + chalk.bold.hex('#A8E6CF')('ngsui add block hero-section') + chalk.hex('#FF6B6B')(' │'));
    console.log(chalk.hex('#FF6B6B')('│  ') + chalk.hex('#FFE66D')('2. Add pricing cards:       ') + chalk.bold.hex('#A8E6CF')('ngsui add block pricing-cards') + chalk.hex('#FF6B6B')('│'));
  ...
  try {
    ...
    for (const pipe of pipesToInstall) {
      try {
        ...
        await fs.writeFile(path.join(pipesDirectory, pipe), fileContent);
      } catch (error) {
        console.warn(chalk.yellow(`Warning: Could not download ${pipe}`));
      }
    }
  } catch (error) {
    console.warn(chalk.yellow('Warning: Could not install pipes'));
  }
}
